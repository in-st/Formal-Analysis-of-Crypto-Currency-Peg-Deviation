...
for i=1:t
    alpha_a_can(:,i) = mvnrnd(Atdraw(:,i),c_a*P_ekf_sm_star_a(:,:,i),1)';
end
...
for i=1:t   
    ...
    l_can_a=l_can_a+log(mvnpdf(alpha_a_can(:,i),alpha_a_ekf(:,i),P_ekf_a(:,:,i)))...
            +log(mvnpdf(alpha_a_can(:,i+1),alpha_a_can(:,i),Sadraw))...
            -log(mvnpdf(alpha_a_can(:,i),Atdraw(:,i),c_a*P_ekf_sm_star_a(:,:,i)))+...
                log(det(reshape(S_A*alpha_a_can(:,i)+s_A,M,M)));

    % Evaluate current state log-density
    l_draw_a=l_draw_a+log(mvnpdf(Atdraw(:,i),alpha_a_ekf(:,i),P_ekf_a(:,:,i)))...
             +log(mvnpdf(Atdraw(:,i+1),Atdraw(:,i),Sadraw))...
                -log(mvnpdf(Atdraw(:,i),alpha_a_can(:,i),c_a*P_ekf_sm_star_a(:,:,i)))+...
                log(det(reshape(S_A*Atdraw(:,i)+s_A,M,M)));
