% "A general algorithm for estimating Structural VARs"
% ------------------------------------------------------------------------------------
% This code implements the SVAR model as in Canova and Pérez Forero (2013)
...
% This version: December 26th, 2013.
% -------------------------------------------------------------------------
...
% Select data horizon
% -------------------
% Data availability: 1959.Q1-2010.Q4
...
test=2; % 1: Short chain. 2: Long chain
...
if TVP_Sigma==1
    diag_sigma=2; % 1. Diagonal Sigma, 2. Garch    
else
...
for p=1:p_bar
    ...
    Sigma_u_tilde(:,:,p)=(1/t)*Y(p+1:t,:)'*(eye(t-p)-Z'*inv(Z*Z')*Z)*Y(p+1:t,:); 
...
if draft==1 % Start from a previous work
    ...
    load([sprintf('%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d.mat',...
            'results_TVP_VAR_MH_final_draft_correct',M,p,irep,nrep+nburn,nburn,nthin,c_b,c_a,k_Q,k_S,k_W,...
            prior,stat,diag_sigma,char(ident_name),single_beta,TVP_Beta,TVP_Alpha,...
            TVP_Sigma,standardize,tcodey(1),sign_restr,horiz,red_dim,Xi_rand)]);
    ...
    if diag_sigma==2    
        if TVP_Sigma==1
            ...
            c_s=1e-3; % This constant re-scales the variance of proposal density               
...
for irep = it_start:nrep + nburn
    ...
    if TVP_Sigma==1
        if diag_sigma==2
            %draw_sigma_TVP_corrected_GARCH1
            draw_sigma_TVP_corrected_GARCH
        else
    ...
    if test>1
        ...
        save([sprintf('%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d.mat',...
            'results_TVP_VAR_MH_final_draft_correct',M,p,irep,nrep+nburn,nburn,nthin,c_b,c_a,k_Q,k_S,k_W,...
            prior,stat,diag_sigma,char(ident_name),single_beta,TVP_Beta,TVP_Alpha,...
            TVP_Sigma,standardize,tcodey(1),sign_restr,horiz,red_dim,Xi_rand)]);
        end
...
%%
save([sprintf('%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%s_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d.mat',...
    'results_TVP_VAR_MH_final_correct_dec13',M,p,nrep+nburn,nburn,nthin,c_b,c_a,k_Q,k_S,k_W,...
    prior,stat,diag_sigma,char(ident_name),single_beta,TVP_Beta,TVP_Alpha,...
    TVP_Sigma,standardize,tcodey(1),sign_restr,horiz,red_dim,Xi_rand,df,delta)]);
