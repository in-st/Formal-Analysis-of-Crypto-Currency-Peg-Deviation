...
for m=1:M
    ...
    P0_s=sigma_prvar(m,m);       % Initial value of var(alpha) (unconditional = 1)    
    Q = sigma0_a_guess^2;% Variance of measurement equation error     
    T_t = delta*sigma0_a_guess;        % Jacobian
    ...
    y_for=yhat2(m,1)-Z1*sigma0_a_guess;
    sigma_a_ekf(1,1) = sigma0_a_guess + K1*(y_for);
    ...
    for i=2:t
        ...
        % Updating:
        Q = sigma_a_ekf(1,i-1)^2; % Variance of measurement equation error
        T_t = delta*sigma_a_ekf(1,i-1);        % Jacobian        
        Z1 = 0;
    ...
    T_t = delta*sigma_a_ekf_sm(1,t);        % Jacobian
    ...
    for i=t-1:-1:1
        ...
        T_t = delta*sigma_a_ekf_sm(1,i);        % Jacobian                
...
for i=1:t   
    ...
    % Evaluate candidate log-density
         ...
         l_can_s=l_can_s+0.5*log(2*pi)+...
                   ...
                   +log(mvnpdf(sigma_a_can(1,i+1)^2,sigma_a_can(1,i)^2+yhat2(m,i)^2,Wdraw(m,m)));
    ...
    % Evaluate current state log-density                  
         ...
         l_draw_s=l_draw_s+0.5*log(2*pi)+...
                   ...
                   +log(mvnpdf(Sigtdraw(m,i+1)^2,Sigtdraw(m,i)^2+yhat2(m,i)^2,Wdraw(m,m)));
    else
